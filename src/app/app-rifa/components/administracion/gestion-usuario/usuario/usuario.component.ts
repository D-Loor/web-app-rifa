...
import { UsuarioService } from 'src/app/app-rifa/services/usuario.service';
import { EncryptedService } from 'src/app/app-rifa/services/utils/encrypted.service';
    ...
    this.usuarioService.actualizarUsuario(this.usuarioForm.value, this.idUsuario).subscribe({
      next: data => {
        if (data['code'] === "200") {
          ...
          this.messageService.add({ key: 'tst', severity: 'error', summary: 'Error!', detail: 'No se pudo actualizar el usuario', life: 3000 });
        }
        this.spinner.hide();
      }, error: e => {
        this.spinner.hide();
